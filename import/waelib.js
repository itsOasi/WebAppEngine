...
let exb_data;
let exb_data_cache;
let comp_folder = "import/static/components/";
let debug = true;

function wae_debug(message){
    debug ? console.debug(message) : null;
}
...
let exhibit = {
    ...
    install_service_workers: function (){
        if ("serviceWorker" in navigator){
            log.write("registering service workers");
            log.write(exb_data);
            for (worker in exb_data.service_workers){
                navigator.serviceWorker.register(exb_data.service_workers[worker]);
            }
        }
        else{
